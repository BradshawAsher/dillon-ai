...
async function run() {
  console.log('=== EXPORTING PROJECT 2 (BUSINESS 2 - IRONTREE) ACTUAL RUN FROM SUPABASE ===')
  ...
  console.log(`Found ${docs?.length || 0} document rows in Supabase for Business 2 / IronTree.`)
  ...
  for (const doc of latestByFile.values()) {
    ...
    formattedDocs.push({
      ...
      fileType: doc.file_type || (doc.file_name.endsWith('.pdf') ? 'pdf' : doc.file_name.endsWith('.xlsx') ? 'xlsx' : 'xltx'),
      status: doc.status,
  ...
  const targetPath = path.join(__dirname, '..', 'test_sets', 'results', 'business2_irontree_actual_run.json')
  fs.writeFileSync(targetPath, JSON.stringify(actualRunFile, null, 2))
  console.log(`✅ Saved Business 2 actual run to: ${targetPath}`)
}
