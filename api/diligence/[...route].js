...
var route_src_exports = {};
__export(route_src_exports, {
  default: () => handler
});
module.exports = __toCommonJS(route_src_exports);
...
function getCitationFromRecord(record) {
...
function getStructuredFindingsFromRaw(raw, fallbackStatus) {
  let value = raw;
  if (typeof value === "string") {
  ...
  return value.map((item) => item && typeof item === "object" ? getStructuredFinding(item, fallbackStatus) : null).filter((item) => item !== null);
}
function getStringListValue(raw, formatObject) {
  let value = raw;
  if (typeof value === "string") {
    const trimmed = value.trim();
...
function formatTakeaway(record) {
...
function formatNegotiationLever(record) {
...
function formatConflict(record) {
...
function extractJudgmentSummary(parsed) {
  if (typeof parsed === "string") return parsed;
  if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
    const record = parsed;
    const response = record.response;
    if (response && typeof response === "object") {
      const responseSummary = getRecordString(response, ["summary", "final_judgment_summary", "finalJudgmentSummary"]);
...
function getJudgmentValues(raw) {
...
function getJudgmentField(raw, field) {
  if (!raw) return void 0;
  try {
    const parsed = JSON.parse(raw);
    if (parsed[field] !== void 0) return parsed[field];
    if (parsed.response && typeof parsed.response === "object" && parsed.response[field] !== void 0) {
      return parsed.response[field];
    }
    return void 0;
  } catch {
    return void 0;
...
function getProjectFlags(raw, key) {
  const camelKey = key === "red_flags" ? "redFlags" : key === "yellow_flags" ? "yellowFlags" : "greenFlags";
  try {
    const parsed = JSON.parse(raw);
    if (parsed.response && typeof parsed.response === "object") {
      const flags = parsed.response.flags;
      if (flags && typeof flags === "object" && flags[key]) {
        return getStringListValue(flags[key], formatFlag);
      }
    }
    if (parsed.flags && typeof parsed.flags === "object" && parsed.flags[key]) {
      return getStringListValue(parsed.flags[key], formatFlag);
    }
...
function getProjectStructuredFlags(raw, key, fallbackStatus) {
  const camelKey = key === "red_flags" ? "redFlags" : key === "yellow_flags" ? "yellowFlags" : "greenFlags";
  try {
    const parsed = JSON.parse(raw);
    if (parsed.response && typeof parsed.response === "object") {
      const flags = parsed.response.flags;
      if (flags && typeof flags === "object" && flags[key]) {
        return getStructuredFindingsFromRaw(flags[key], fallbackStatus);
      }
    }
    if (parsed.flags && typeof parsed.flags === "object" && parsed.flags[key]) {
      return getStructuredFindingsFromRaw(parsed.flags[key], fallbackStatus);
    }
...
function getJudgmentRecommendation(raw) {
  const recommendation = getJudgmentField(raw, "final_recommendation");
  if (recommendation && typeof recommendation === "object") {
    return getRecordString(recommendation, ["recommendation"]);
  }
  return typeof recommendation === "string" ? recommendation : "";
}
function getCitationDetails(judgmentJson, aiCitations) {
  const found = [];
...
async function getProjectSynthesis(req) {
  ...
  const { data: rows, error } = await query.order("id", { ascending: false }).limit(limitNum);
...
// backend/diligence/saveDealModel.ts
...
async function saveDealModel(req) {
  const projectId = req.params.projectId?.trim();
  ...
  const row = {
    ...
    model_updated_at: (/* @__PURE__ */ new Date()).toISOString(),
    model_updated_by: req.user.email ?? ""
  };
  const { data, error } = await supabase.from("deal_models").upsert(row, { onConflict: "project_id" }).select().single();
  if (error) throw new Error(`Supabase write failed: ${error.message}`);
  return data;
}

// backend/diligence/getSubmissionHistory.ts
var STORAGE_CDN_URL = (process.env.VITE_STORAGE_CDN_URL || process.env.STORAGE_CDN_URL || "https://dillon-ai-worker.bradshin231.workers.dev").replace(/\/+$/, "");
var SUPABASE_STORAGE_ORIGIN = "https://sihpsqrunkwkxhhnwoqe.supabase.co";
function resolveStorageCdnUrl(url) {
...
var activeSubmissionStatuses = /* @__PURE__ */ new Set([
  "uploading",
  "accepted",
  "queued",
  "processing",
...
async function syncPendingRowsFromN8nDataTable(rows) {
    ...
    for (const pendingRow of pendingRows) {
      ...
      if (match && String(match.status ?? "").toLowerCase() === "completed") {
        const updatePayload = {
          ...
          processed_at: match.ai_processedAt || (/* @__PURE__ */ new Date()).toISOString(),
...
function unpackExtractedFallback(row) {
  let parsed = null;
...
async function getSubmissionHistory(req) {
  ...
  const { data: rows, error } = await query.order("updated_at", { ascending: false }).limit(limitNum);
  if (error) throw new Error(`Supabase read failed: ${error.message}`);
  if (!rows) return [];
  ...
  try {
    ...
    if (reportPath) {
      ...
      return [
        {
          id: "local-latest",
          run_at: report.evaluatedAt || (/* @__PURE__ */ new Date()).toISOString(),
          commit_sha: "HEAD",
...
async function getProjectActionTracker(req) {
  const projectId = req.params.projectId?.trim();
  if (!projectId) throw new Error("projectId is required");
  const { data: rows, error } = await supabase.from("project_action_trackers").select("*").eq("project_id", projectId).limit(1);
  if (error) throw new Error(`Supabase read failed: ${error.message}`);
...
// backend/diligence/saveProjectActionTracker.ts
async function saveProjectActionTracker(req) {
  const projectId = req.params.projectId?.trim();
  if (!projectId) throw new Error("projectId is required");
  const row = {
    project_id: projectId,
    ...
    last_modified_at: (/* @__PURE__ */ new Date()).toISOString(),
    last_modified_by: req.user.email ?? ""
  };
  const { data, error } = await supabase.from("project_action_trackers").upsert(row, { onConflict: "project_id" }).select().single();
  if (error) throw new Error(`Supabase write failed: ${error.message}`);
  return data;
}

// backend/diligence/submissionPayload.ts
function getSubmitPath(environment) {
  return environment === "test" ? "webhook-test/d6884691-1689-479d-b1b3-ee7a8bca7380" : "webhook/d6884691-1689-479d-b1b3-ee7a8bca7380";
}
function normalizeSubmitEnvironment(environment) {
  return environment === "test" ? "test" : "production";
}
function normalizeWebhookResponse(response, fallback) {
  return {
...
async function storedFileMultipart(entries, signal) {
  ...
  const cleanup = async () => {
    for (const path2 of paths) {
      await (0, import_promises2.unlink)(path2).catch((error) => {
        if (error.code !== "ENOENT") throw error;
      });
      paths.delete(path2);
    }
    if (directory) {
      await (0, import_promises2.rmdir)(directory);
      directory = void 0;
    }
  };
  try {
    for (const entry of entries) {
      ...
      if (entry.fileUrl) {
        ...
        let bytes = 0;
        try {
          directory ??= await (0, import_promises2.mkdtemp)((0, import_node_path.join)((0, import_node_os.tmpdir)(), "mergeworks-handoff-"));
          const path2 = (0, import_node_path.join)(directory, `attachment-${paths.size}`);
          paths.add(path2);
          await (0, import_promises.pipeline)(
            import_node_stream.Readable.fromWeb(source.body),
            new import_node_stream.Transform({ transform(chunk, _encoding, callback) {
              bytes += chunk.length;
            ...
            (0, import_node_fs.createWriteStream)(path2, { flags: "wx", mode: 384 }),
            { signal }
          );
          ...
          signal.throwIfAborted();
          const fileBuffer = await (0, import_promises2.readFile)(path2);
...
// backend/diligence/retryFailedDocument.ts
async function retryFailedDocument(req) {
  ...
  const path2 = environment === "test" ? "webhook-test/dd-retry-failed-document" : "webhook/dd-retry-failed-document";
...
// backend/diligence/stopProjectSynthesis.ts
async function stopProjectSynthesis(req) {
  const projectId = req.params.projectId?.trim();
  if (!projectId) throw new Error("projectId is required");
  const { error } = await supabase.from("project_syntheses").update({
...
// backend/diligence/updateSubmissionRow.ts
async function updateSubmissionRow(req) {
  const requestID = req.params.requestID?.trim();
  if (!requestID) throw new Error("requestID is required");
  ...
  const { error } = await supabase.from("documents").update({ is_considered: isConsidered }).eq("request_id", requestID);
  if (error) throw new Error(`Supabase write failed: ${error.message}`);
  return { ok: true };
}
...
async function handleAccessRequest(req) {
  ...
  const requestId = import_node_crypto.default.randomUUID ? import_node_crypto.default.randomUUID() : `req_${Date.now()}`;
...
function installBackendGlobals() {
...
function getClientIp(headers) {
  ...
  const forwardedParts = Array.isArray(forwarded) ? forwarded.flatMap((part) => part.split(",")) : typeof forwarded === "string" ? forwarded.split(",") : [];
...
function rateLimit(ip, route, method) {
  ...
  if (timestamps.length >= limit) {
    ...
    const retryAfterSec = Math.max(1, Math.ceil((WINDOW_MS - (now - oldest)) / 1e3));
...
function sendJson(req, res, status, body, cacheControl) {
  ...
  const etag = `"${import_node_crypto2.default.createHash("md5").update(jsonString).digest("hex")}"`;
