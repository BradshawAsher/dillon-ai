...
async function stopBatchSubmission(req) {
  const requestIDs = Array.isArray(req.params.requestIDs) ? req.params.requestIDs.map((value) => String(value).trim()).filter((value) => value.length > 0) : [];
  if (requestIDs.length === 0) {
    throw new Error("requestIDs is required");
  }
  const path2 = req.params.environment === "test" ? "webhook-test/dd-document-consideration" : "webhook/dd-document-consideration";
...
var DEFAULT_SLACK_WEBHOOK = "https://hooks.slack.com/services/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";
