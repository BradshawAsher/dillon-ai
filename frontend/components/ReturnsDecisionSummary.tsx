                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    <Kpi
                        ...
                        statusBadge={
                            <DataOriginBadge
                                origin="calculated"
                                label={priceIsConfirmed ? "Calculated (Confirmed Price)" : "Calculated (Preview Price)"}
                                metricLabel="Cash needed at close"
                                metricValue={money(results.initialInvestment, currency)}
                                formula="purchase price + closing fees + working capital"
                                description="Total upfront cash required at closing to purchase the business and fund initial liquidity."
                                compact
                            />
                        }
                    />
                    <Kpi
                        ...
                        statusBadge={
                            <DataOriginBadge
                                ...
                                metricLabel="Annual cash generated"
                                metricValue={money(results.annualCashFlow, currency)}
                                formula="EBITDA/SDE × (1 − tax rate) − maintenance capex"
                                description="Net cash flow generated by operations after paying taxes and essential maintenance capex."
                                compact
                            />
                        }
                    />
                    <Kpi
                        ...
                        statusBadge={
                            <DataOriginBadge
                                origin="calculated"
                                label={ebitdaIsConfirmed ? "Calculated (Verified Payback)" : "Calculated (Illustrative)"}
                                metricLabel="Payback period"
                                metricValue={results.paybackYears === null ? 'Needs input' : `${results.paybackYears.toFixed(1)} years`}
                                formula="initial investment ÷ annual operating cash flow"
                                description="Operating timeline required to recover 100% of upfront capital from business cash flow."
                                compact
                            />
                        }
                    />
                    <Kpi
                        ...
                        statusBadge={
                            <DataOriginBadge
                                origin="calculated"
                                label={ebitdaIsConfirmed && priceIsConfirmed ? "Calculated (Verified Exit)" : "Calculated (Illustrative Preview)"}
                                metricLabel="Total return with sale"
                                metricValue={results.exitReady ? `${results.totalMoic?.toFixed(2) ?? '—'}x / ${results.irr === null ? '—' : `${(results.irr * 100).toFixed(1)}%`}` : 'Add exit inputs'}
                                formula="IRR(all cash flows), MOIC((cumulative cash + net exit) ÷ investment)"
                                description="Total multiple on capital and internal rate of return across the planned holding period and final sale."
                                compact
                            />
                        }
                    />
