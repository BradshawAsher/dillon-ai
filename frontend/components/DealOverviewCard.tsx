...
export default function DealOverviewCard({ syntheses, projects, currentProjectId, askingPrice, onAskingPriceChange, impact, model, documents, onOpenEvidence, exampleMode = false, onSwitchTab }: DealOverviewCardProps) {
    ...
    const usableFact = (field: string) => {
        const fact = documentedFacts[field]
        return (fact?.status === 'confirmed' || fact?.status === 'illustrative' || fact?.status === 'reported' || fact?.status === 'estimated') && typeof fact?.value === 'number'
            ? fact.value ?? null
            : null
    }
    ...
    const revenue = usableFact('revenue')
    const ebitda = usableFact('ebitda_sde')
    const debt = usableFact('debt')
    const assets = usableFact('total_assets')
    ...
    const kpis = [
        ...
        { label: 'Payback period', value: paybackYears === null ? 'Not available' : `${paybackYears.toFixed(1)} years`, detail: 'Initial investment ÷ annual operating cash flow', source: exampleMode ? 'Example data' : (documentedFacts.ebitda_sde?.status === 'confirmed' ? 'Documented + assumptions' : 'Estimated + assumptions'), evidence: evidenceForFact('ebitda_sde', 'Payback-period input evidence') },
                    ...
                    <>
                        ...
                        <div className="grid gap-3 lg:grid-cols-3">
                            ...
                            <div className="rounded-lg border border-border bg-background p-4">
                                ...
                                <p className="mt-1 text-sm leading-6 text-muted-foreground">{synthesis.missingDocuments.length > 0 ? `${synthesis.missingDocuments.length} requested material${synthesis.missingDocuments.length === 1 ? '' : 's'} still missing (same list shown in the Synthesis tab).` : 'No missing core materials recorded.'}</p>
                            </div>
                        </div>
