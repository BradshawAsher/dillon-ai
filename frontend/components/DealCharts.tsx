            ...
            <div className="mt-4 h-64" role="img" aria-label={`${title}. ${description}`}>
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <BarChart data={data} margin={{ top: 28, right: 8, left: 8, bottom: 0 }}>
                        ...
                        <Bar dataKey={dataKey} name="Value" radius={[5, 5, 0, 0]} isAnimationActive={false}>
            ...
            <div className="mt-2 h-64" role="img" aria-label={`${title}: annual cash flow by year`}>
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <BarChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
                        ...
                        <YAxis tickFormatter={compactMoney} tickLine={false} axisLine={false} width={64} tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} />
                        <Tooltip content={<ChartTooltip />} cursor={{ fill: 'hsl(var(--muted))' }} />
                        <Bar dataKey="cashFlow" name="Cash flow" radius={[5, 5, 0, 0]} isAnimationActive={false}>
                            {data.map((entry) => (
            ...
            <div className="mt-2 h-64" role="img" aria-label="Cumulative cash flow and payback timeline">
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <LineChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
                        ...
                        <Line type="monotone" dataKey="cumulativeCashFlow" name="Cumulative cash flow" stroke={isVerified ? chartColors[0] : '#f59e0b'} strokeWidth={2.5} dot={{ r: 3 }} isAnimationActive={false} />
                    </LineChart>
                </ResponsiveContainer>
            </div>
        </div>
...
export function GrowthLineChart({ data }: { data: ChartDatum[] }) {
    ...
    return (
        <div className="rounded-xl border border-border bg-muted/20 p-4">
            <p className="text-sm font-semibold">Revenue path by case</p>
            <p className="mt-1 text-xs text-muted-foreground">Projected revenue from the documented starting point and saved scenario assumptions. Bear is red/dashed, Base is blue/solid, and Bull is green/dotted.</p>
            <div className="mt-4 h-64" role="img" aria-label="Projected bear red dashed, base blue solid, and bull green dotted revenue by year">
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <LineChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                        <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} />
                        ...
                        <Line type="monotone" name="Bear (downside)" dataKey="Bear" stroke="#dc2626" strokeWidth={2.5} strokeDasharray="7 4" dot={false} activeDot={{ r: 4 }} isAnimationActive={false} />
                        <Line type="monotone" name="Base (expected)" dataKey="Base" stroke="#2563eb" strokeWidth={3} dot={false} activeDot={{ r: 4 }} isAnimationActive={false} />
                        <Line type="monotone" name="Bull (upside)" dataKey="Bull" stroke="#16a34a" strokeWidth={2.5} strokeDasharray="2 4" dot={false} activeDot={{ r: 4 }} isAnimationActive={false} />
                    </LineChart>
                </ResponsiveContainer>
            </div>
        </div>
...
export function EbitdaLineChart({ data }: { data: ChartDatum[] }) {
    ...
    return (
        <div className="rounded-xl border border-border bg-muted/20 p-4">
            <p className="text-sm font-semibold">EBITDA projection by case</p>
            <p className="mt-1 text-xs text-muted-foreground">Projected EBITDA (revenue x margin) for each scenario. Bear is red/dashed, Base is blue/solid, and Bull is green/dotted.</p>
            <div className="mt-4 h-64" role="img" aria-label="Projected bear red dashed, base blue solid, and bull green dotted EBITDA by year">
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <LineChart data={data} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                        <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} />
...
export function WaterfallChart({ title, description, data }: { title: string; description: string; data: WaterfallDatum[] }) {
    ...
    return (
        <div className="rounded-xl border border-border bg-muted/20 p-4">
            ...
            <div className="mt-4 h-72" role="img" aria-label={`${title}. ${description}`}>
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    <BarChart data={computedData} margin={{ top: 24, right: 8, left: 8, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                        <XAxis dataKey="label" tickLine={false} axisLine={false} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} interval={0} angle={-20} textAnchor="end" height={50} />
                        <YAxis tickFormatter={compactMoney} tickLine={false} axisLine={false} width={64} tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} />
                        <Tooltip content={<ChartTooltip />} cursor={{ fill: 'hsl(var(--muted))' }} />
                        <Bar dataKey="base" stackId="stack" fill="transparent" name="" isAnimationActive={false} />
                        <Bar dataKey="value" stackId="stack" name="Value" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                            <LabelList dataKey="value" position="top" formatter={(v) => compactMoney(Number(v))} fontSize={11} fill="hsl(var(--foreground))" />
                            {computedData.map((entry) => <Cell key={entry.label} fill={entry.fill} />)}
                        </Bar>
                    </BarChart>
                </ResponsiveContainer>
            </div>
