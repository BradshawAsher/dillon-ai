                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <CardInfoPopover cardId="project-checklist" />
                        </div>
                        <CardDescription className="mt-1">Optional analyst checklist for {projectId || 'this project'}. It does not block document processing or synthesis.</CardDescription>
                    </div>
                    <Badge variant={completed === items.length ? 'success' : 'outline'}>{completed}/{items.length} complete</Badge>
                </div>
            </CardHeader>
            <CardContent className="space-y-3 p-4">
                <div className="flex gap-2 border-b border-border pb-3">
                    <button type="button" onClick={() => setView('active')} className={view === 'active' ? 'rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground' : 'rounded-md px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted'}>To do ({items.length - completed})</button>
                    <button type="button" onClick={() => setView('completed')} className={view === 'completed' ? 'rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground' : 'rounded-md px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted'}>Completed ({completed})</button>
                </div>
                {visibleItems.length ? visibleItems.map((item) => (
                    <label key={item.id} className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-background p-3 transition-colors hover:bg-muted/30">
                        <Switch checked={Boolean(state[item.id])} onCheckedChange={(checked) => onChange({ ...state, [item.id]: checked })} />
                        <span className="min-w-0 flex-1">
                            <span className="flex flex-wrap items-center gap-2">
                                <span className="text-sm font-medium text-foreground">{item.label}</span>
                                {item.recommended ? <Badge variant="warning" className="text-[10px]">Recommended</Badge> : null}
                                {state[item.id] ? <CheckCircle2 className="h-4 w-4 text-success" /> : null}
                            </span>
                    ...
                    <p className="rounded-lg border border-dashed border-border bg-muted/20 p-4 text-sm text-muted-foreground">
                        {view === 'completed' ? 'No checklist items have been completed yet.' : 'All checklist items are complete. You can move an item back from the Completed tab if needed.'}
                    </p>
                )}
            </CardContent>
        </Card>
