    ...
    return <Card className="overflow-hidden">
        <CardHeader className="border-b border-border bg-card/80">
            <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <DataOriginBadge
                    origin={isIllustrativePreview ? "assumption" : "calculated"}
                    label={isIllustrativePreview ? "Illustrative Capital Stack" : "Saved Capital Stack"}
                    formula="Sources (Debt + Equity + Seller Note) = Uses (Price + Fees + Working Capital)"
                />
            </div>
        ...
        <CardContent className="space-y-4 p-5">
            <div className="rounded-xl border-2 border-primary bg-gradient-to-br from-primary/15 via-primary/5 to-background p-5 shadow-md">
                <p className="text-sm font-bold uppercase tracking-wide text-primary">Start here — deal structure at a glance</p>
                ...
                <p className="mt-4 text-sm leading-6 text-foreground">The buyer funds {uses > 0 ? `${((equity / uses) * 100).toFixed(0)}%` : '—'} of the deal with equity and the rest with debt. {dscr === null ? 'Add confirmed EBITDA/SDE to test whether the business can comfortably carry that debt.' : `Operating cash flow covers the planned debt service ${dscr.toFixed(2)}x.`}</p>
            </div>
