...
export default function ManagementQuestionTracker({ projectId, suggestedQuestions }: { projectId: string; suggestedQuestions: string[] }) {
    ...
    useEffect(() => {
        try {
            const saved = JSON.parse(window.localStorage.getItem(keyFor(projectId)) || '[]') as Question[]
            const existing = new Set(saved.map((item) => item.question.trim().toLowerCase()))
            setQuestions([...saved, ...suggestedQuestions.filter((question) => !existing.has(question.trim().toLowerCase())).map(createQuestion)])
        } catch { setQuestions(suggestedQuestions.map(createQuestion)) }
    }, [projectId, suggestedQuestions])
