...
export function buildMemoText(model: DealModel, synthesis: ProjectSynthesisItem | undefined, projectName: string): string {
    ...
    lines.push('---')
    lines.push('Generated by Dillon AI Due Diligence Dashboard')
    return lines.join('\n')
}
        ...
        <Card className="overflow-hidden print:border-0 print:shadow-none">
            ...
            <CardContent className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-lg border border-border p-3 flex flex-col justify-between">
                        <div className="flex items-center justify-between gap-1">
                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground font-medium">Revenue</p>
                            <DataOriginBadge
                                origin={typeof revenue === 'number' ? 'extracted' : 'assumption'}
                                label={typeof revenue === 'number' ? 'Confirmed Fact' : 'Illustrative'}
                                metricLabel="Revenue"
                                metricValue={typeof revenue === 'number' ? money(revenue) : '$1.2M'}
                                formula="Sum of trailing twelve months (TTM) gross recognized revenues"
                                description="Top-line gross sales before cost of goods sold, operating expenses, and taxes."
                                compact
                            />
                        </div>
                    ...
                    <div className="rounded-lg border border-border p-3 flex flex-col justify-between">
                        <div className="flex items-center justify-between gap-1">
                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground font-medium">EBITDA/SDE</p>
                            <DataOriginBadge
                                origin={typeof ebitda === 'number' ? 'extracted' : 'assumption'}
                                label={typeof ebitda === 'number' ? 'Confirmed Fact' : 'Illustrative'}
                                metricLabel="EBITDA/SDE"
                                metricValue={typeof ebitda === 'number' ? money(ebitda) : '$175K'}
                                formula="Operating Income (EBIT) + Depreciation + Amortization + Discretionary Owner Add-backs"
                                description="Seller's Discretionary Earnings (SDE) or EBITDA representing the total operational cash flow generated by the business."
                                compact
                            />
                        </div>
                    ...
                    <div className="rounded-lg border border-border p-3 flex flex-col justify-between">
                        <div className="flex items-center justify-between gap-1">
                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground font-medium">Price</p>
                            <DataOriginBadge
                                origin={typeof price === 'number' ? 'user_entered' : 'assumption'}
                                label={typeof price === 'number' ? (model.purchasePrice ? 'Purchase Price' : 'Asking Price') : 'Illustrative'}
                                metricLabel="Purchase Price"
                                metricValue={typeof price === 'number' ? money(price) : '$1.0M'}
                                formula="Target purchase consideration agreed upon or offered for the acquisition"
                                description="Total enterprise purchase price for acquiring 100% equity of the business."
                                compact
                            />
                        </div>
                        {typeof price === 'number' ? (
                    ...
                    <div className="rounded-lg border border-border p-3 flex flex-col justify-between">
                        <div className="flex items-center justify-between gap-1">
                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground font-medium">Multiple</p>
                            <DataOriginBadge
                                origin="calculated"
                                label="Calculated Multiple"
                                metricLabel="Entry Multiple"
                                metricValue={multiple ? `${multiple}x` : '5.7x'}
                                formula="Purchase Price ÷ EBITDA/SDE"
                                description="Valuation entry multiple indicating how many years of current cash flow the buyer is paying to purchase the company."
                                compact
                            />
                        </div>
                        {multiple ? (
