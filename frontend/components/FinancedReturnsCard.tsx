...
function Metric({ label, value, detail, evidence, onOpenEvidence, infoTerm }: { label: string; value: string; detail: string; evidence: EvidenceItem; onOpenEvidence?: (evidence: EvidenceItem) => void; infoTerm?: string }) {
    return <div className="rounded-lg border border-border bg-background p-3">
        <p className="text-xs text-muted-foreground">{label}{infoTerm && FINANCIAL_TERMS[infoTerm] ? <InfoTip term={infoTerm} definition={FINANCIAL_TERMS[infoTerm]} /> : null}</p>
...
export default function FinancedReturnsCard({ model, documents = [], onOpenEvidence }: { model: DealModel; documents?: SubmissionHistoryItem[]; onOpenEvidence?: (evidence: EvidenceItem) => void }) {
    ...
    return <Card className="overflow-hidden border-primary/30">
        ...
        <CardContent className="space-y-4 p-5">
            <div className="rounded-lg border border-primary/25 bg-primary/5 p-4"><p className="text-sm font-semibold">Quick read</p><p className="mt-1 text-sm leading-6 text-muted-foreground">{!ready ? 'Add price and EBITDA/SDE to see how much equity the buyer needs and whether operating cash flow can carry the debt.' : <>The buyer contributes {money(equity!)} at close and retains {money(cashAfterDebt!)} after annual debt service. {dscr !== null ? `Debt coverage is ${dscr.toFixed(2)}x${dscr < 1.25 ? ', which is below the usual 1.25x screening threshold' : ', indicating operating cash flow covers scheduled debt service'}.` : ''}{exitReady && totalMoic !== null ? ` The modeled exit produces ${totalMoic.toFixed(2)}x on equity.` : ''}</>}</p></div>
            {!ready ? <div className="rounded-lg border border-dashed border-border bg-muted/20 p-4 text-sm text-muted-foreground">Add documented EBITDA/SDE, price, tax rate, equity contribution, interest rate, and amortization term.</div> : <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Metric label="Equity at close" value={money(equity!)} detail="uses − debt − seller note" evidence={evidence('Equity at close', 'equity = total uses − senior debt − seller note')} onOpenEvidence={onOpenEvidence} infoTerm="Equity contribution" />
                <Metric label="Annual debt service" value={money(annualDebtService!)} detail="amortizing loan payment" evidence={evidence('Annual debt service', 'annual debt service is the level payment calculated from senior debt, interest rate, and amortization term')} onOpenEvidence={onOpenEvidence} infoTerm="Amortization" />
                ...
                <Metric label="Cash-on-cash return" value={coc === null ? 'Not available' : `${(coc * 100).toFixed(1)}%`} detail="cash after debt ÷ equity" evidence={evidence('Cash-on-cash return', 'cash-on-cash return = cash after debt service ÷ equity at close')} onOpenEvidence={onOpenEvidence} infoTerm="Cash-on-cash" />
                <Metric label="Debt-service coverage (DSCR)" value={dscr === null ? 'Not available' : `${dscr.toFixed(2)}x`} detail="operating cash flow ÷ debt service" evidence={evidence('Debt-service coverage ratio', 'DSCR = operating cash flow ÷ annual debt service')} onOpenEvidence={onOpenEvidence} infoTerm="DSCR" />
