...
export default function ValuationImpactBridge({ synthesis, baseValue, onOpenEvidence }: { synthesis: ProjectSynthesisItem; baseValue: number | null; onOpenEvidence?: (item: EvidenceItem) => void }) {
    ...
    }, [synthesis.crossDocumentConflicts, synthesis.structuredFindings.crossDocumentConflicts])
    ...
    return <Card className="overflow-hidden"><CardHeader className="border-b border-border bg-card/80"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><Scale className="h-5 w-5 text-primary" /><CardTitle className="text-xl">Evidence-linked value bridge</CardTitle></div><CardDescription className="mt-1">Translate evidence into a proposed price or terms mechanism. Amounts are analyst-entered assumptions, never LLM-generated values.</CardDescription></div><Badge variant="warning">Analyst review required</Badge></div></CardHeader><CardContent className="space-y-4 p-4">{items.length === 0 ? <p className="rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">No cross-document conflicts are available to translate into a price or terms adjustment.</p> : <><div className="space-y-3">{items.map((item) => <div key={item.id} className="grid gap-3 rounded-lg border border-border bg-muted/20 p-3 lg:grid-cols-[minmax(0,1fr)_minmax(220px,0.65fr)_160px]"><button type="button" onClick={() => onOpenEvidence?.({ title: 'Value-bridge evidence', sourceFile: item.sourceFile || synthesis.citations?.[0] || 'Project synthesis', sourceLocation: item.sourceLocation || 'Project-level synthesis', excerpt: item.excerpt || item.finding, confidence: item.confidence ?? undefined, status: item.status || 'Synthesized', provenance: 'Project synthesis' })} className="text-left text-sm leading-6 text-foreground hover:text-primary">{item.finding}<span className="ml-2 text-xs font-medium text-primary">View evidence</span></button><select value={item.mechanism} onChange={(event) => setItems((current) => current.map((candidate) => candidate.id === item.id ? { ...candidate, mechanism: event.target.value } : candidate))} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option>Price reduction or seller note</option><option>Purchase-price adjustment / closing adjustment</option><option>Debt payoff or escrow</option><option>Earn-out or diligence condition</option><option>Diligence condition / analyst review</option></select><label className="flex items-center gap-2 rounded-md border border-input bg-background px-3"><span className="text-xs text-muted-foreground">$</span><Input value={item.amount} onChange={(event) => setItems((current) => current.map((candidate) => candidate.id === item.id ? { ...candidate, amount: event.target.value } : candidate))} inputMode="decimal" placeholder="Amount" className="border-0 px-0 focus-visible:ring-0" /></label></div>)}</div><div className="grid gap-3 rounded-lg border border-primary/20 bg-primary/[0.04] p-4 sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">Base value</p><p className="mt-1 text-lg font-semibold text-foreground">{baseValue === null ? 'Pending' : formatCurrencyValue(String(baseValue), synthesis.valuationCurrency || 'USD')}</p></div><div><p className="text-xs text-muted-foreground">Bridge adjustments</p><p className="mt-1 text-lg font-semibold text-foreground">{adjustmentTotal === 0 ? '$0' : `-${formatCurrencyValue(String(adjustmentTotal), synthesis.valuationCurrency || 'USD')}`}</p></div><div><p className="text-xs text-muted-foreground">Illustrative adjusted value</p><p className="mt-1 text-lg font-semibold text-foreground">{adjustedBase === null ? 'Pending' : formatCurrencyValue(String(adjustedBase), synthesis.valuationCurrency || 'USD')}</p></div></div><div className="flex items-center gap-3"><button type="button" disabled={saving} onClick={handleSaveBridge} className="rounded-md bg-primary px-3 py-2 text-sm font-medium text-white">{saving ? 'Saving…' : 'Save bridge to Deal Model'}</button>{saveError ? <p className="text-sm text-destructive">{saveError}</p> : null}</div></>}</CardContent></Card>
}
