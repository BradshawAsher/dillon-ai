...
describe('manualDealIntake utilities', () => {
    ...
    describe('buildManualDealModel', () => {
        it('produces a complete DealModel with Senior Debt and Equity check', () => {
            ...
            expect(model.equityContributionPercent).toBe(0.20)
            expect(model.interestRate).toBe(0.095)
            expect(model.taxRate).toBe(0.25)
            expect(model.baseRevenueGrowth).toBe(0.075)
            expect(model.baseEbitdaMargin).toBe(0.225)
            expect(model.equityAmount).toBe(960000)
            expect(model.sellerNoteAmount).toBe(500000)
            expect(model.exitCosts).toBe(Math.round(1_110_000 * 4.8 * 0.04))
