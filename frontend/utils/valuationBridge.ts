...
export function computeValuationBridge(
    ...
    const reportedEbitda = facts.ebitda_sde?.isOverridden && typeof facts.ebitda_sde.originalAiValue === 'number'
    ...
    if (synthesis?.redFlags && synthesis.redFlags.length > 0) {
        synthesis.redFlags.forEach((flag, idx) => {
            ...
            if (lower.includes('contractor') || lower.includes('1099') || lower.includes('tax') || lower.includes('payroll')) {
                category = 'liability'
                baseAmount = extractedDollar || 150_000
                ...
                category = 'capex'
                baseAmount = extractedDollar || 120_000
                ...
                baseAmount = extractedDollar || Math.round(normalizedEbitda * 0.4)
                ...
                category = 'liability'
                baseAmount = extractedDollar || 75_000
    ...
    // If no red flags with dollars exist, provide a baseline standard indemnity escrow item
    if (items.length === 0 && baselinePrice > 0) {
        const standardEscrow = Math.round(baselinePrice * 0.05)
        const id = 'bridge-general-escrow'
        items.push({
            id,
            title: 'Baseline General Indemnity Escrow (R&W Coverage)',
            category: 'liability',
            baseAmount: standardEscrow,
            totalDeduction: standardEscrow,
            handling: itemHandlingOverrides[id] || 'special_escrow',
            rationale: 'Standard 5% purchase price indemnity escrow held for 18 months post-closing.',
            evidenceSource: 'Market Standard PE Terms',
            apaSectionRef: 'Section 8.2',
        })
    }
    ...
    const totalSavingsDollars = totalEvDeduction + totalSpecialEscrow + totalEarnoutContingent
...
export function generateSpecialEscrowClause(bridge: ValuationBridgeResult, companyName: string): string {
    ...
    const totalEscrow = bridge.totalSpecialEscrow > 0 ? bridge.totalSpecialEscrow : Math.round(bridge.baselinePurchasePrice * 0.05)
