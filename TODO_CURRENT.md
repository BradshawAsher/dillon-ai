...
- Need to assess the structure and order of overview and and analysis page and maybe break them up since they are getting long, or just re-order them or something?
...
- [x] Add repeatable regression fixtures/mock projects for the quantitative cases, including missing inputs, mismatched periods, and negative/downside outcomes. Created `frontend/utils/fixtures.ts` with 5 scenarios (healthy deal, high-growth SaaS, distressed/turnaround, minimal data, zero EBITDA) plus `frontend/utils/cardCalculations.test.ts` exercising BaseReturnMetrics, DealStack, and GrowthSensitivity calculations across all fixtures with NaN/Infinity guards.
...
- [x] Add a management-question tracker with owner, priority, status, response, and resulting thesis impact. Interactive DealActionItemsCard and SellerQuestionsCard now support custom items, completion checkboxes, priority tags, delete actions, Markdown exports, and per-project localStorage persistence (`projectActionTracker.ts`).
...
- [x] Consider independent second-pass LLM review only after deterministic checks, with explicit comparison and review flags rather than silent overwrites. SecondOpinionCard upgraded with an interactive **"Run AI Forensic Audit"** execution flow, anomaly flag detection, item verification toggles, completeness score gauge, and per-project localStorage persistence.
- [-] Obtain external test sets and create additional realistic mock diligence packages. Added 3 more fixtures (Professional Services Firm, Franchise, E-commerce) to `frontend/utils/fixtures.ts` for a total of 8 scenarios. External real-world test documents still needed.
- [x] Tax Planning Impact. TaxImpactCard shows impact of 4 tax strategies (asset step-up, Section 179, interest deduction, pass-through election) with annual and 5-year cumulative savings. Added to Returns tab.
- [x] Public Data Enrichment. PublicDataEnrichmentCard upgraded with interactive target domain enrichment, digital footprint score, tech stack detection, public sentiment rating, and per-project localStorage persistence.

- [x] Operating Leverage. OperatingLeverageCard shows degree of operating leverage, fixed cost ratio, and scenario table projecting EBITDA impact from +5% to +20% revenue growth. Added to Growth tab.
- [x] Working Capital Analysis. WorkingCapitalCard shows DSO, inventory days, net working capital, WC/revenue ratio, and annual growth WC needs with health indicators. Added to Deal Structure tab.
...
- [x] Monte Carlo Simulation. MonteCarloCard runs 1,000 scenarios varying growth (±3%), margin (±3%), and exit multiple (±1.0x) with seeded deterministic random. Shows MOIC distribution histogram (5 buckets), loss probability, 2x/3x chances, and P10/P25/P50/P75/P90 percentile outcomes. Added to Returns tab and Deep Analysis.
- [x] Negotiation Impact Calculator. NegotiationImpactCard shows potential dollar savings from common negotiation levers (10% price reduction, seller note, reduced equity, working capital shift, earnout, consulting credit). Ranked by savings with difficulty badges (easy/medium/hard). Added to Deep Analysis Negotiation & Closing section.
- [x] Breakeven Analysis. BreakevenAnalysisCard shows visual bars comparing current performance to minimum thresholds: EBITDA to cover debt service, revenue breakeven, exit EBITDA for 1x MOIC, and DSCR. Green/red color coding with margin percentages. Added to Returns tab and Deep Analysis.
- [x] Payback Timeline Chart. PaybackTimelineCard shows a 10-year cumulative cash flow bar chart (green above breakeven, red below). Accounts for revenue growth and highlights the payback year. Shows total investment, payback year, and year-10 value. Added to Returns tab.
- [x] Owner Dependency Risk Score. OwnerDependencyCard scans synthesis flags for 5 owner/founder dependency indicators (client relationships, revenue generation, management team, licenses/certifications, supplier relationships). Scores 0-100 with per-factor mitigation strategies. Added to Deep Analysis Risk Assessment section.
- [x] Term Sheet Generator. TermSheetCard auto-generates a draft term sheet from model assumptions: target info, financing structure (equity/debt/seller note), key terms, conditions, and open issues from red flags. Monospace formatted with copy-to-clipboard. Added to Negotiation & Closing section.
- [x] SMB Benchmark Comparison. BenchmarkComparisonCard shows this deal vs typical SMB acquisitions on 5 dimensions (entry multiple, EBITDA margin, revenue growth, payback period, DSCR) with colored range bars, dot positioning, and letter grades (Excellent/Good/Below avg/Poor). Added to Deep Analysis.
- [x] Deep Analysis promoted to its own tab. Moved from a sub-tab under Overview to its own "Analysis" tab in the workspace nav. Users can now access it directly without scrolling. Overview now always shows the Summary view (Deal Memo, KPIs, Grade, Scores, Stats, Valuation, Radar, Action Items).
- [x] Persistent KPI bar at page top. Added a compact KPI strip above the workspace nav showing: project name, risk signal (with flag count), entry multiple, EBITDA margin, document count, and data quality. Always visible regardless of which tab is active.
- [x] Annual Cash Flow Breakdown. AnnualCashFlowCard shows a waterfall: EBITDA → taxes → after-tax income → maintenance capex → debt service → net cash flow to owner. Bidirectional bars with final annual amount and % of EBITDA reaching the owner. Added to Returns tab.
- [x] Exit Readiness Assessment. ExitReadinessCard scores 6 exit-preparedness factors: growth trajectory, achievable exit multiple, MOIC potential, risk profile for sale, management independence, and margin profile. Shows projected exit value and MOIC. Added to Growth tab.
- [x] Debt Service Coverage Analysis. DebtServiceCoverageCard shows current DSCR gauge with threshold markers (1.0, 1.25, 1.5, 2.0), revenue decline tolerance percentage before DSCR drops below 1.0x, and EBITDA decline tolerance before hitting 1.25x lender minimum. Color-coded status. Added to Returns tab.
- [x] Deal Timing Optimizer. DealTimingCard determines best close month for fiscal year alignment, calculates days until next quarter end, provides seasonal working capital adjustment recommendation, and shows tax optimization close timing. Added to Analysis Negotiation & Closing section.
- [x] Value Creation Plan. ValueCreationPlanCard shows a 5-step value creation roadmap: revenue growth, margin improvement, working capital optimization, debt paydown, and multiple expansion. Each step has dollar impact estimate, timeline, and difficulty. Shows total value creation and ROI on effort. Added to Growth tab.
- [x] Command Palette: Analysis tab entry added. Fixed deal-structure tab routing to use correct 'structure' identifier. Added Diligence and History tab entries with appropriate icons.
- [x] Diligence Completeness Card. DiligenceCompletenessCard tracks analysis thoroughness across 4 categories (Financial Data, Valuation Inputs, Risk Assessment, Deal Structure) with 20 checkpoints, progress bars, and next-steps callout. Added to Analysis Risk Assessment section.
- [x] Year 1 Monthly Projection. WeeklyProjectionCard shows month-by-month cumulative cash position for year 1 post-acquisition. Net cash flow after taxes, capex, and debt service with bidirectional bars and summary metrics. Added to Returns tab.
- [x] Leverage Safety Margin. LeverageSafetyCard stress-tests the deal at 0-50% EBITDA declines, showing DSCR, Debt/EBITDA, and breach status at each level. Shows safety cushion percentage and warns when margin is thin. Added to Deal Structure tab.
- [x] Deal on a Page. DealOnAPageCard provides a screenshot-friendly one-glance deal summary: price, revenue, EBITDA, entry multiple, financing structure bar, red/green flag counts, projected MOIC, and risk signal. Added to top of Analysis tab.
- [x] Sensitivity Ranking. SensitivityRankingCard computes MOIC impact of ±10% changes to 6 key assumptions, ranks by absolute swing with tornado bars, and highlights the top 2 most impactful drivers for focused diligence. Added to Returns tab.
- [x] Acquisition Timeline. AcquisitionTimelineCard shows a vertical 8-milestone timeline from LOI to Transition with status auto-inferred from available data and total estimated weeks to close. Added to Analysis Negotiation & Closing section.
- [x] Risk vs Reward Scatter. RiskRewardScatterCard plots the deal on a 2D risk-reward grid with 4 labeled quadrants. Risk score from flags, reward from projected MOIC. Added to Analysis Risk Assessment section.
- [x] Quick Wins. QuickWinsCard identifies highest-impact improvements ranked by effort: negotiate multiple, seller financing, margin improvement, working capital, red flag leverage, rate shopping. Added to Analysis Insights section.
- [x] Downside Protection. DownsideProtectionCard assesses 6 structural safeguards: entry multiple buffer, seller alignment, manageable leverage, operational margin, working capital cushion, equity stake. Added to Deal Structure tab.
- [x] Cash Reserve Analysis. CashReserveAnalysisCard shows recommended post-acquisition cash reserves with priority levels (critical/recommended/optional): operating expenses, debt service, working capital, capex, emergency fund. Added to Deal Structure tab.
- [x] Investor/Lender Readiness. InvestorReadinessCard checks 8 criteria for deal package readiness: financial docs, revenue, EBITDA, synthesis, price, financing, risks, growth assumptions. Added to Analysis Negotiation section.
- [x] First 100 Days Plan. First100DaysCard generates a 3-phase post-acquisition roadmap (Stabilize/Optimize/Grow) with auto-generated action items from red flags and deal characteristics. Added to Growth tab.
- [x] Deal Scorecard Export. DealScorecardExportCard shows formatted scorecard with executive summary, key metrics, risk assessment, financing, and Go/No-Go recommendation. Copy to clipboard. Added to Analysis tab top.
- [x] Market Position. MarketPositionCard classifies the deal across 4 dimensions (size, profitability, growth, pricing) with visual tier indicators and overall summary. Added to Analysis after benchmarks.
- [x] What-If Scenarios. WhatIfScenariosCard shows 5 real-world scenarios (best case, base case, revenue stalls, lose customer, recession) with MOIC, IRR, and verdict for each. Added to Returns tab.
- [x] Key Metrics Trajectory. KeyMetricsTrendCard projects revenue, EBITDA, enterprise value, remaining debt, and equity MOIC at Year 3 and Year 5 with trend indicators. Added to Growth tab.
- [x] Financing Structures Compared. FinancingComparisonCard shows 4 common financing approaches side-by-side: All Cash, Conservative 60/40, SBA 10/80/10, Seller-friendly 20/50/30 with visual bars, equity needed, monthly debt, cash-on-cash, and DSCR. Added to Deal Structure tab.
- [x] Comparable Transactions. ComparableTransactionsCard generates hypothetical market comps at different multiples with implied prices and premium/discount vs asking. Includes typical SMB range reference grid. Added to Valuation tab.
- [x] Deal Killer Check. DealKillerCheckCard tests 8 common deal-breaker conditions (DSCR, entry multiple, red flags, open questions, margins, leverage, growth, price/revenue). Pass/fail with explanations and overall verdict. Added to Analysis Risk Assessment.
- [x] EBITDA Quality Score. EBITDAQualityScoreCard grades EBITDA quality (A-D) across 5 dimensions with progress bars and detail explanations. Added to Analysis after FinancialHealthCard.
