...
- Eventually, we will have to have data be stored per-person, like maybe we can store locally before the user creates an account, but if people are testing this tool, then we don't want them to be able to see other people's data? Can we make admin account for ourselves to see all data and then set up a way that testers can only see their data? Or do we already have this implemented? Can we also add a toggle for this to be on/off depending on what we are testing?
