...
| **Tier 5: 3-Tier Deal Copilot** | Conversational Tool-Calling Copilot | Interactive chat agent equipped with sliding deal memory, native browser Web Speech API voice dictation, microsecond client-side financial calculation tools, and citation drawer/card anchor deep-linking for buy-side investment committees. | Streaming $< 200\text{ms}$ TTFT |
...
* **Interactive Data Lineage & Verification Tiers (`DataOriginBadge`)**:
  - Implements a 4-tier verification hierarchy: Tier 1 (Document Confirmed via single citation), Tier 2 (Confirmed & Verified via multi-document concordance within 2% tolerance), Tier 3 (Confirmed & Reconciled via deterministic arithmetic accounting identities), and Tier 4 (Confirmed & Calculated via financial modeling formulas).
  - Every financial metric in the dashboard features interactive formula popovers showing mathematical formulas, source citations, and direct "Ask AI" context links.
* **Native Browser Voice Dictation & Local Tools**: Leverages the browser Web Speech API (`webkitSpeechRecognition`) inside `DealChatPanel` for zero-latency, local speech-to-text dictation with visual pulsing feedback, paired with microsecond client-side tool execution (`executeClientSideTool`) for instant arithmetic answers.
* **Direct Storage Uploads (R2-Primary)**: The client requests tickets via `/api/diligence/upload-url`. **Cloudflare R2** (`dillon-deal-documents` bucket, $0 egress) is the primary upload target for all file sizes. Files larger than 6 MiB fall back to signed resumable Supabase uploads in 6 MiB chunks only if R2 upload fails. Only files at most 3 MiB may fall back to inline base64 after all storage paths fail. All backend document downloads route through the Cloudflare Worker CDN edge proxy (`resolveCdnStorageFetchUrl`) with 1-year immutable caching to eliminate Supabase storage egress.
...
1. **Unstructured Multi-Modal VDR Ingestion (Cloud Pipeline)**:
   - For complete diligence rooms: uploaded files are streamed directly to **Cloudflare R2 Object Storage** ($0 egress), triggering n8n OCR, multi-model extraction (Terra/Sol), and cross-document synthesis.
...
| **Data Lineage Badges & Verification Tiers** | Provides full mathematical auditability and page citations (Tier 1-4) across every financial metric so investment committees can verify all numbers. | *Black-box LLM summaries*: Rejected because institutional buyers and lenders refuse unverified AI text without audit provenance. |
| **Asynchronous Decoupled Ingestion (524 Resilience)** | Decouples upload registration from background LLM execution, returning a 202 Accepted response and updating state via Supabase Realtime CDC and compact polling. | *Long-lived synchronous HTTP connections*: Rejected because Cloudflare and Vercel edge proxies kill requests exceeding 100s with HTTP 524 Gateway Timeouts. |
| **Client-Side Voice Dictation (Web Speech API)** | Native browser speech recognition executes locally with zero cloud latency, zero egress cost, and pulsing visual state. | *Streaming raw audio to Whisper cloud API*: High latency, costly per-second audio billing, and potential confidentiality leaks. |
| **Pre-Render HTML Sanitization (DOMPurify)** | Assistant markdown in chat and memos is escaped and purified before DOM insertion to neutralize stored XSS attacks hiding in scanned financial text. | *Unsanitized markdown rendering*: High security vulnerability to prompt-injected script tags from malicious seller files. |
...
7. **Asynchronous 524 Edge Timeout Decoupling**: Large financial packets requiring multi-minute OCR or synthesis run asynchronously in n8n Cloud and write directly to Supabase PostgreSQL. The client never maintains an open HTTP connection, polling lightweight metadata (`full=false`, <1.2 KB) or receiving Supabase Realtime CDC events to prevent Cloudflare 524 Gateway Timeouts.
8. **Zero-Division & Infinite Re-render Guards**: All financial ratios and margin calculations use defensive zero-denominator guards (`safeDiv`) preventing `NaN%` and `Infinity%` that break SVG charts. TanStack Query uses memoized selectors and ref-based watchdog timers to eliminate React Error #185 re-render cascades.
9. **Stored XSS Sanitization & Prompt Injection Defense**: Pre-render HTML escaping and `DOMPurify` protect `DealChatPanel` and the Deal Memo from malicious payloads in scanned document text, with zero-storage ephemeral memory for user BYOK API keys.
