...
4. [Complete] Replace Returns placeholder with all-cash + financed calculator. All-cash and financed baseline calculations now use persisted Deal Model inputs; financing assumptions include equity contribution, interest rate, amortization, and seller note. With a hold period, exit multiple, and exit costs, both models calculate terminal proceeds, total MOIC, and deterministic IRR from their annual cash-flow series.
5. [Complete] Add bear/base/bull returns and scenario comparison. Persisted scenario inputs now drive transparent all-cash revenue, EBITDA margin, annual operating cash-flow paths, exit value, operating payback, MOIC, and IRR comparisons. Financed bear/base/bull modeling remains a later enhancement.
6. [Complete] Add valuation-method comparison and sensitivity. Asset-based, revenue-multiple, EBITDA/SDE-multiple, and blended methods use confirmed facts plus saved assumptions. The Valuation tab now includes a compact exit-value grid across revenue growth, EBITDA margin, and exit-multiple variations. Quantified valuation-bridge adjustments remain a later enhancement.
