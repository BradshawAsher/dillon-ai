-- If n8n retries after the database committed but the HTTP response was lost,
-- return the original ownership token to that same execution.
...
begin
    ...
    v_execution_id := nullif(btrim(coalesce(p_execution_id, '')), '');
    v_signature := encode(
    ...
    select
        existing.status = 'running'
            ...
            and existing.execution_id is not distinct from v_execution_id,
        existing.id,
        case
            when existing.status = 'running'
             and existing.lease_expires_at > now()
             and existing.execution_id is not distinct from v_execution_id
            then existing.claim_token
            else null::uuid
        end,
        existing.status,
